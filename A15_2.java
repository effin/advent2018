...
public class A15_2 {
    ...
    private static int elvesPower = 0;
...
//        Scanner scanner = new Scanner(new File("input15_t0.txt"));
//        Scanner scanner = new Scanner(new File("input15_t1.txt"));
        ...
        IntStream.range(0, input.size()).forEach(i -> {
            original[i] = input.get(i).toCharArray();
            map[i] = input.get(i).toCharArray();
        });

        do {
            ++elvesPower;
            System.out.println(elvesPower);
            elvesDeath = false;
            play();
        } while (elvesDeath);
    ...
    static boolean elvesDeath;

    static void play() {
        rounds = 0;
        IntStream.range(0, original.length).forEach(i -> System.arraycopy(original[i], 0, map[i], 0, original[0].length));
        hitpoints = new int[map.length][map[0].length];
        while (!elvesDeath) {
            hasMoved = new boolean[map.length][map[0].length];
...
//        int[] moveTo = moveToList.stream().sorted((a, b) -> a[0] == b[0] ? Integer.compare(a[1], b[1]) : Integer.compare(a[0], b[0])).findFirst().get();
    ...
    static void getDistances(int i, int j, int[][] distances) {
        final char me = map[i][j];
        for (int k = 0; k < distances.length; k++) {
            Arrays.fill(distances[k], MAX_VALUE);
        }
        distances[i][j] = 0;
        walk(i, j, distances);
    }
...
//        if (distances[i][j] == 1) {
        ...
        hitpoints[targeti][targetj] += getPower(map[i][j]);
